...
Page({
    ...
    money: function(t) {
        var a;
        console.log(t.detail.value), a = "" != t.detail.value ? t.detail.value : 0, this.setData({
            money: parseFloat(a).toFixed(2)
        });
    },
    formSubmit: function(a) {
        var e = this;
        form_id = a.detail.formId, e.setData({
            form_id: form_id
        });
        var o = this.data.userinfo.openid, t = this.data.userinfo.id, s = this.data.money, i = this.data.storeinfo.store.id;
        if (console.log(o, t, s, i), 0 == s) return wx.showModal({
            title: "提示",
            content: "付款金额不能等于0"
        }), !1;
        if (console.log("form发生了submit事件，携带数据为：", a.detail.value.radiogroup), "yezf" == a.detail.value.radiogroup) {
        ...
        if ("yezf" == a.detail.value.radiogroup) var d = 2;
        if ("wxzf" == a.detail.value.radiogroup) d = 1;
        if ("jfzf" == a.detail.value.radiogroup) d = 3;
        console.log("pay_type", d), "" == form_id ? wx.showToast({
            ...
            success: function(t) {
                e.setData({
                    zfz: !1,
                    showModal: !1
                }), console.log(t), "下单失败" != t.data && ("yezf" == a.detail.value.radiogroup ? console.log("余额支付流程") : "jfzf" == a.detail.value.radiogroup ? console.log("积分支付流程") : (console.log("微信支付流程"), 
                app.util.request({
                    url: "entry/wxapp/pay",
                    cachetime: "0",
                    data: {
