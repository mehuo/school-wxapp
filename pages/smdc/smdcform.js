...
Page({
                ...
                console.log(o), "2" == a.storeset.is_zt && s.setData({
                    ...
                    success: function(t) {
                        console.log(t), app.util.request({
                            ...
                            cachetime: "0",
                            success: function(t) {
                                console.log(t.data), 1 == t.data ? s.setData({
    ...
    formSubmit: function(e) {
        ...
        var a = this.data.tableid, n = e.detail.formId, d = this.data.form_id2, r = wx.getStorageSync("users").id, l = this.data.store.id, c = this.data.totalPrice, u = this.data.totalyh, h = this.data.gwcinfo.box_money, g = this.data.mjmoney, p = this.data.xyhmoney, f = parseInt(this.data.selectedindex) + 1, m = this.data.note, y = this.data.cjarray[this.data.cjindex], w = this.data.yhqkdje, v = this.data.activeradio, _ = this.data.hbactiveradio, b = this.data.hbkdje;
        if (console.log("桌子id", a, s, n, d, r, l, "实付", c, "总优惠", u, "包装费", h, "满减金额", g, "新用户money", p, "优惠券", w, "红包", b, "订单类型", f, "留言", m, "sz", o, "餐具数量", y), 
        "yezf" == e.detail.value.radiogroup) var x = 2;
        if ("wxzf" == e.detail.value.radiogroup) x = 1;
        if ("jfzf" == e.detail.value.radiogroup) x = 3;
        if ("chzf" == e.detail.value.radiogroup) x = 5;
        console.log("支付方式", x), "" == n ? wx.showToast({
            title: "没有获取到formid",
            icon: "loading",
            duration: 1e3
        }) : (this.setData({
